...
const faqItems = [
  {
    ...
    answer:
      "รับงานด่วนได้ แจ้งจำนวนใบและวันต้องการใช้งาน เราจัดคิวและส่งกลับให้ทันรอบไลฟ์ ส่วนใหญ่ 2-4 วันทำการขึ้นกับจำนวน",
  },
  {
    question: "คิดราคายังไง ถ้าเหมาเยอะลดได้ไหม?",
    answer:
      "คิดตามจำนวนใบ เหมายิ่งเยอะราคายิ่งถูก (เริ่ม 14 บาท/ใบ สำหรับ 500 ใบขึ้นไป) งานแก้ทรงหนักหรือแบรนด์พิเศษประเมินเพิ่มจากรูปได้",
  },
  {
    question: "ส่งหมวกมายังไง ต้องแยกสีไหม?",
    answer:
      "แยกโทนสีและทรงใส่ถุง เพื่อลดสีตก/กดทับทรง เขียนชื่อร้าน เบอร์โทร ที่อยู่ส่งกลับ และจำนวนด่วนไว้หน้ากล่อง",
  },
  {
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-800">
      ...
      <main className="pt-20">
        ...
        <QueueSection queues={queues} loading={loading} />
        <FAQSection />
        {error ? (
  ...
  if (loading && !articles.length) {
    return (
      <section id="articles" className="bg-gradient-to-b from-green-50 to-white py-16">
        <div className="mx-auto max-w-6xl px-4">
          <p className="text-gray-500">กำลังโหลดบทความ...</p>
        </div>
      </section>
    );
  }
...
function QueueSection({ queues, loading }: { queues: QueueEntry[]; loading: boolean }) {
  if (loading && !queues.length) {
    return (
      <section className="bg-white py-16">
        <div className="mx-auto max-w-6xl px-4">
          <p className="text-gray-500">กำลังโหลดคิว...</p>
        </div>
      </section>
    );
  }
